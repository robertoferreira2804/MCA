...
if (!requireNamespace("Matrix", quietly = TRUE)) {
  stop("Package 'Matrix' is required. Please install it via install.packages('Matrix').")
}
library(Matrix)
...
# -----------------------------
w_eff <- min(w, d - 1L)
nnz_est <- d + (n - 1) * d * (w_eff + 1) + sum(in_control_mid)  # numeric
slack <- 1.15
nnz_cap <- ceiling(nnz_est * slack)  # numeric
nnz_cap <- min(nnz_cap, .Machine$integer.max)
nnz_cap <- as.integer(max(nnz_cap, 1000))
